...
import { undiciFetch } from './undici-http';
...
import { logInfo, logWarn } from './sora-logger';
...
export async function generateViaExternalChat(
  ...
  const { getPricingConfig } = await import('./db/system-config-pricing');
  const cost = resolveVideoGenerationCost(await getPricingConfig(), request, model);
  return {
    type: 'sora-video',
    url: rawUrl,
    cost,
