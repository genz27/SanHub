...
export async function getEnabledVisionChatModel(): Promise<ChatModel | null> {
  return withCache(`${CacheKeys.CHAT_MODELS}runtime:vision`, CacheTTL.CHAT_MODELS, async () => {
    await ensureDatabase();
    const db = getAdapter();
    const [rows] = await db.execute(
      `SELECT id, name, api_url, api_key, model_id, supports_vision, max_tokens, enabled, cost_per_message
       FROM chat_models
