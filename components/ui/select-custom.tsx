        ...
        <SelectPrimitive.Value placeholder={placeholder}>
          {selectedOption && (
            <div className="flex flex-col items-start text-left leading-tight">
              <span className={cn("font-medium block truncate", selectedOption.highlight && "text-foreground")}>
            ...
            {options.map((option) => (
              <SelectPrimitive.Item
                ...
                className={cn(
                  ...
                  option.highlight && "bg-accent focus:bg-accent"
                )}
              >
                <div className="flex items-center gap-2">
                  {option.highlight && <Sparkles className="w-3.5 h-3.5 text-foreground shrink-0" />}
                  <span className={cn("font-medium", option.highlight && "text-foreground")}>
                    {option.label}
                  </span>
                </div>
